###This script will perform the tetrachoric correlation for the POND dataset which addresses the first aim of the paper 
...
Exc_T1w <- rbind(StandVis_Exc, StandVisMet_Exc, StringVis_Exc, StringVisMet_Exc, Automated_Exc)
...
library(janitor)
Exc_T1w_Groups <- tabyl(Exc_T1w, ID, Exc_Group)
...
library(psych)
Mixed_cor_T1_QC_inc <- mixedCor(Exc_T1w_Groups, d=c(1:5)) 
Mixed_cor_T1_QC_inc_DF <- as.matrix(Mixed_cor_T1_QC_inc$rho)

#3. Plotting the correlation matrix
library(corrplot)
library(RColorBrewer)
col23 <- colorRampPalette(c("#BB4444", "#EE9988", "#FFFFFF", "#77AADD", "#4477AA"))
Mixed_cor_T1_QC_inc_corplot <- corrplot.mixed(Mixed_cor_T1_QC_inc_DF, lower.col = "#BB4444")
